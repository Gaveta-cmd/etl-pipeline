        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-[0.1em] text-muted">
              ...
              <th className="px-4 py-3 text-right font-medium">Lidos</th>
              <th className="px-4 py-3 text-right font-medium">Carregados</th>
              <th className="px-4 py-3 text-right font-medium">Descartados</th>
            </tr>
          </thead>
          <tbody>
            <AnimatePresence initial={false}>
              {runs.length === 0 ? (
                ...
                runs.map((run) => (
                  ...
                  >
                    <td className="px-4 py-3.5 font-display font-semibold text-brand">#{run.runId}</td>
                    <td className="px-4 py-3.5"><StatusBadge status={run.status} /></td>
                    <td className="px-4 py-3.5 text-muted">{formatDateTime(run.startTime)}</td>
                    <td className="px-4 py-3.5 text-right text-muted tabular-nums">{formatDuration(run.durationMs)}</td>
                    <td className="px-4 py-3.5 text-right tabular-nums text-ink">{formatNumber(run.recordsRead)}</td>
                    <td className="px-4 py-3.5 text-right font-medium tabular-nums text-brand">{formatNumber(run.recordsLoaded)}</td>
                    <td className="px-4 py-3.5 text-right tabular-nums text-clay">{formatNumber(run.recordsSkipped)}</td>
                  </motion.tr>
                ))
              )}
            </AnimatePresence>
